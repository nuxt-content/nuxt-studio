...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    const isCloudinaryMedia = Boolean(isExternalMediaEnabled && options.media?.provider === 'cloudinary')
    if (isExternalMediaEnabled && !isCloudinaryMedia) {
    ...
    // Public runtime config. Never expose Cloudinary credentials to the browser.
    const { cloudinary: cloudinaryOptions, ...publicMediaOptions } = options.media || {}
    nuxt.options.runtimeConfig.public.studio = {
      ...
      i18n: options.i18n,
      // @ts-expect-error Autogenerated type does not match with options
      media: { ...publicMediaOptions, external: isExternalMediaEnabled },
    ...
    nuxt.options.runtimeConfig.studio = {
      ...
      media: {
        external: Boolean(isExternalMediaEnabled),
        provider: options.media?.provider || 'nuxthub',
        publicUrl: options.media?.publicUrl || '',
        maxFileSize: options.media?.maxFileSize || 10 * 1024 * 1024,
        allowedTypes: options.media?.allowedTypes || ['image/*', 'video/*', 'audio/*'],
        prefix: options.media?.prefix || 'studio',
        cloudinary: {
    ...
    if (isCloudinaryMedia) {
      await setExternalMediaStorage(nuxt, runtime, 'cloudinary')
    }
    else if (isExternalMediaEnabled) {
      await setExternalMediaStorage(nuxt, runtime, 'nuxthub')
    }
    else {
