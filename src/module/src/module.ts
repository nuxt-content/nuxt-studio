import { defineNuxtModule, createResolver, addPlugin, addServerPlugin, extendViteConfig, addServerHandler, addServerImports, useLogger, hasNuxtModule } from '@nuxt/kit'
...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.runtimeConfig.studio = {
      ...
      auth: {
        sessionSecret: '',
        github: { ...(options.auth?.github || {}), redirectUrl: '', moderators: '' } as never,
        gitlab: { ...(options.auth?.gitlab || {}), redirectUrl: '', moderators: '' } as never,
        google: { ...(options.auth?.google || {}), redirectUrl: '', moderators: '' } as never,
        sso: { ...(options.auth?.sso || {}), redirectUrl: '' } as never,
      },
      git: {
    ...
    addServerPlugin(runtime('./server/plugins/studio-env'))
