...
Cloudinary can be used as the external media provider without adding a NuxtHub dependency:
...
Configure `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, and `CLOUDINARY_API_SECRET` as server-only environment variables. Optionally set `CLOUDINARY_FOLDER`; the default folder is `studio`. Cloudinary media is uploaded, listed, and deleted through Studio's authenticated server route and is not committed to Git.
