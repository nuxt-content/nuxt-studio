...
:::tip
Default instance URL is dynamic based on the provider:
- With GitHub provider: default instance URL is `https://github.com`.
- With GitLab provider: default instance URL is `https://gitlab.com`.
:::
...
export default defineNuxtConfig({
  studio: {
    repository: {
      // ...
      instanceUrl: 'https://github.company.com' // or 'https://gitlab.company.com' when using GitLab provider
    }
  }
})
```

For self-hosted GitLab, also configure the auth provider with the same instance URL:
