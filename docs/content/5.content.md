...
##### Global `iconLibraries` in `nuxt.config`

You can restrict **every** Studio icon picker (collection forms and Vue component props in the visual editor) by setting `studio.iconLibraries` in `nuxt.config.ts`. Values are Iconify collection prefixes, same as the field-level option above.

Per-field `iconLibraries` in `.editor()` **overrides** the global list when set, so you can keep a default set project-wide and narrow further for specific fields.
