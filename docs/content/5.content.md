...
You can control which components are visible in Nuxt Studio using the `editor.components` option in your `nuxt.config.ts`. This is useful if you want to hide specific components or only show a subset.
...
When you have many components, you can organize them into custom groups in the slash command menu:
  ...
  studio: {
    editor: {
      components: {
        ...
        exclude: [],
        groups: [
          { label: 'Content', include: ['content*', 'app/components/content/**'] },
          { label: 'Landing', include: ['landing*', 'app/components/landing/**'] },
          { label: 'UI', include: ['ui*', 'app/components/ui/**'] },
        ],
        ungrouped: 'include',
      }
    }
...
The `/` menu includes built-in TipTap sections (**Style** and **Insert**) for headings, lists, marks, images, and so on. `editor.components` only filters **Vue components**; to hide these built-in entries, use `editor.commands.exclude` — see [Commands](/setup#commands).
