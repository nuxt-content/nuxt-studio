...
All components in the `/components/content` folder are automatically available. You can also filter which components are available in the editor.
...
You can control which components are visible in Nuxt Studio using the `components` option in your `nuxt.config.ts`. This is useful if you want to hide specific components or only show a subset.
