...
::note{type="info"}
AI features are optional. They require either a Vercel AI Gateway API key or a Google Gemini API key. Vercel provides $5 in free credits, which is typically sufficient for most editing scenarios over several days.
::
...
- Powered by a fast model (Claude Haiku 4.5 via Vercel AI Gateway, or `geminiFastModel` via Gemini) for quick responses
...
- Powered by a quality model (Claude Sonnet 4.5 via Vercel AI Gateway, or `geminiModel` via Gemini) for high-quality results
