...
The TipTap visual editor provides a modern Notion-like editing experience for Markdown content, powered by the popular [TipTap](https://tiptap.dev/) editor and seamlessly integrated with [Nuxt UI Editor](https://ui.nuxt.com/pro/components/editor).
...
### Working with `MDC` Components (`Vue` components)

The visual editor fully supports MDC (Markdown Components):

1. **Inserting Components** - Type `/` and select from the list of available components
2. **Editing Slots** - Edit component slots directly in the visual editor
3. **Managing Props** - Component properties can be edited (*visual props editor coming in beta*)
4. **Inline Components** - Supports both block and inline MDC components
...
- [MDC](/docs/files/markdown){target="_blank"} syntax for `Markdown` files
