...
Plug Studio into your own auth (password form, SSO, etc.) and create the session directly with the provided utilities.

**Flow**

1. In your existing login handler (or a new one, e.g. `/api/studio/login`), validate the user as you normally would.
2. Call `setStudioUserSession(event, user)` with a `StudioUser`-shaped object. This sets the session and the client-side cookie Studio relies on.
   - Required fields: `provider`, `providerId`, `accessToken`, `name`, `email`
   - The `accessToken` must allow repo writes for the configured provider (GitHub/GitLab token or equivalent).
3. For logout, call `clearStudioUserSession(event)` in your current logout handler (or a dedicated route) to clear the session and cookie.
...
export default eventHandler(async (event) => {
  ...
  if (password !== process.env.STUDIO_TEST_PASSWORD) {
    return { error: 'Unauthorized' }
  }

  await setStudioUserSession(event, {
    provider: 'github',          // or 'gitlab' / 'google'
    providerId: 'custom-user',
...
After setting the session, redirect users to your app root (`/`) and Studio will activate automatically for that user.